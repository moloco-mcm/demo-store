...
export type GetRecommendationApiSuccessResponse =
  ApiSuccessResponse<ResponseBody>;
export type GetRecommendationApiErrorResponse = ApiErrorResponse;
export type GetRecommendationApiResponse = ApiResponse<ResponseBody>;
...
export const isValidGetRecommendationRequestBody = (
  data: any
): data is GetRecommendationApiRequestBody =>
  REQUEST_BODY_SCHEMA.isValidSync(data, {
    strict: true,
    abortEarly: true,
  });

export const postHandler: NextApiHandler<GetRecommendationApiResponse> = async (
  req,
  res
) => {
  const { body } = req;
  ...
  if (isAsyncTryCatchError(decisionApiResult)) {
    ...
    return res.status(500).json(
      apiStandardError('INTERNAL_SERVER_ERROR', {
        message: error.message || 'Failed to fetch recommendation result',
      })
    );
  }
